...
Index 5 - NGO status 
    1 - not an NGO client
    2 - NGO client, < 2 years as a client
    3 - NGO client, >= 2 years as a client
...
%run gender, incarceration, homelessness and age model to steady state
SS = ones([1,1*2*2*4*2*1*1*3*1]);
SS = SS./sum(sum(sum(sum(sum(sum(sum(sum(sum(SS)))))))))*params.PWID_initial_pop_size;
...
SS = y(end,:);
SS = reshape(SS,[1,2,2,4,2,1,1,3,1]);
switch model 
    case 'Calibration'
    IC = zeros(1,2,2,4,3,4,8,3,1);
    length_IC = 1*2*2*4*3*4*8*3*1;
    length_other_outputs = 7; %%%%%%%%%%% TO INCLUDE PROP SEX
    case 'Projections'
    IC = zeros(2,2,2,4,3,4,8,3,7);
    length_IC = 2*2*2*4*3*4*8*3*7;
    length_other_outputs = 18; %%%%%%%%%%%% TO INCLUDE PROP SEX
end
