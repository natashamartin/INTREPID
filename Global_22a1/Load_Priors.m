...
priors.RR_HCV_NSP = make_lognormal_dist([0.24,0.09,0.62]);%%%%%%%%%%%%%% EDITS FROM ANTOINE   NEEDED FOR HCV AS WELL

% relative risk of HCV transmission through idu if on OST
priors.RR_HCV_OST = make_lognormal_dist([0.50,0.4,0.63]);
...
% infections that spontaneous clear if HIV negative
priors.alpha_HIV_neg = make_normal_dist([0.2537,0.2176,0.2924]);

% infections that spontaneous clear if HIV positive
priors.alpha_HIV_pos_OR = make_lognormal_dist([0.58,0.38,0.88]);
...
% Increase in HCV infectivty if HIV co-infected
priors.RR_HCV_HIV = makedist('uniform',1,7);
...
% 0.5 - 2 * average duration of injecting
...
% HCV transmission rate through IDU
priors.lambda_HCV_inj = makedist('uniform',0,0.5);
...
% increase in ART recruitment rate if in contact with NGO
% priors.RR_ART_rec_NGO = makedist('uniform',1,5);
...
% RR of starting OST if in contact with NGO
% priors.RR_OST_start_non_NGO = makedist('uniform',0,1);
%% Homeless start

% rate of starting contact with homless
% priors.NGO_start = makedist('uniform',0,1);
...
% reduction in new PWID
% priors.factor_reduction_initiation = makedist('uniform',0,1);
% priors.factor_reduction_date = makedist('uniform',1990,2000);
% priors.factor_reduction_length = makedist('uniform',0,10);

% priors.PWID_initial_pop_size = make_uniform_dist(ISO,'PWID_initial_pop_size');
