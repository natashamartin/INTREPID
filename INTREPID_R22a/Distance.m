...
[model,Diff,Diff_vec] = Distance3(Get_Parameters(samp_params,ISO,'Calibration'),tmp.Data);
...
sum(sum(y<0));
add_outs = y(:,1*2*2*4*3*4*8+1:end);

y_out = reshape(y(:,1:1*2*2*4*3*4*8), [length(t),1,2,2,4,3,4,8]);
a=size(add_outs);

b=size(y(:,1:1*2*2*4*3*4*8));
c=size(y_out);
...
% timestep
% 
% aa=[0; diff(add_outs(:,2))];
% size(aa)
% bb=[timestep; diff(TIME')];
% size(bb)
% 
% yy=[0; diff(add_outs(:,2))]./[timestep; diff(TIME')];
% yy
abc=sum(sum(sum(sum(sum(sum(y_out(:,1,:,:,:,:),3),4),5),6),7),8);
...
model = struct('TIME',TIME,...
    ...
    sum(y_out(:,1,:,:,[1,3,4],:,:,:),3:8)*100,...
     'PWID_prop_recent_inc',sum(y_out(:,1,:,:,3,:,:,:),3:8)./... 
    sum(y_out(:,1,:,:,[1,3,4],:,:,:),3:8)*100,...
    ...
    'RR_HIV_Incidence_rec_inc', [0; diff(add_outs(:,2))]./[timestep; diff(TIME')],... %timestep,...  % [timestep; diff(TIME')],... % using diff(TIME) rather than timestep because data may not conform to 0.1 year timesteps
    'RR_HIV_Incidence_nonrec_inc', [0; diff(add_outs(:,3))]./[timestep; diff(TIME')],... %timestep,...  %[timestep; diff(TIME')],...
    'RR_HIV_Incidence_homeless', [0; diff(add_outs(:,4))]./[timestep; diff(TIME')],... %timestep,...  %[timestep; diff(TIME')],...
    ...
    'PWID_prop_HIV_sexual', [0; diff(add_outs(:,1))]./[timestep; diff(TIME')]); %timestep); %[timestep; diff(TIME')]); % using diff(TIME) rather than timestep because data may not conform to 0.1 year timesteps
%%
...
% ind = (t==2015);
% Prop_young  = sum(sum(sum(sum(sum(sum(y_out(ind,1,:,1,[1,3,4],:,:,:),3),4),5),6),7),8)./...
%     sum(sum(sum(sum(sum(sum(y_out(ind,1,:,:,[1,3,4],:,:,:),3),4),5),6),7),8)*100;
...
%Diff_OST(1) = Difference_sqrd10(field);
...
%Diff_ART(1) = Difference_sqrd10(field);
...
Diff_vec=struct('Diff_prop_young',Diff_prop_young,...
    ...
    'Diff_OST',Diff_OST,...
    'Diff_ART',Diff_ART,...
    'Diff_HIV_sexual',Diff_HIV_sexual);
% size( [sum(Diff_pop_size) + sum(Diff_prop_young)+ sum(Diff_ever_inc) + sum(Diff_homeless)+ ...
%    sum(Diff_HIV_OR) + sum(Diff_HIV_sexual)])
...
% Diff = [sum(Diff_pop_size) + sum(Diff_prop_young), sum(Diff_ever_inc) + sum(Diff_homeless), ...
%    sum(Diff_HIV_OR) + sum(Diff_HIV_sexual),...
%    sum(Diff_OST) + sum(Diff_ART)];
% 
% Diff = sum(Diff);
...
%     end
    ...
    function x = Difference_unif(field)
      time_ind = ismember(TIME,Data.(char(field)).time_pt);
        m = model.(char(field));
        est = Data.(char(field)).estimate;
        l = Data.(char(field)).lower;
        ...
        %x = sum(abs(m(time_ind,:) - est)./est);
        %
        x = sum((1-(m(time_ind,:)>=l).*(m(time_ind,:)<=u)).*999999);
